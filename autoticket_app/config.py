...
DEFAULT_OUTLOOK_INBOX_SUBFOLDER = "AutoTicket"
...
class OutlookSettings:
    inbox_subfolder: str = DEFAULT_OUTLOOK_INBOX_SUBFOLDER
...
def _parse_outlook_settings(data: Any) -> OutlookSettings:
    if not isinstance(data, dict):
    ...
    if not isinstance(outlook_data, dict):
        raise ConfigError("'outlook' must be an object")

    inbox_subfolder = outlook_data.get("inbox_subfolder", DEFAULT_OUTLOOK_INBOX_SUBFOLDER)
    if not isinstance(inbox_subfolder, str) or not inbox_subfolder.strip():
        raise ConfigError("'outlook.inbox_subfolder' must be a non-empty string")

    return OutlookSettings(inbox_subfolder=inbox_subfolder.strip())
